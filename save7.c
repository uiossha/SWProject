...
struct space{
  ...
  double fi[NXY];/*the function phi*/
  double q[NXY]; /*the q in the equation*/
  double xc[NX];/*x center*/
  double yc[NY];/*y center*/
  int li [NX]; /*line*/
  int x[NX];
  int y[NY];
};
...
void SIPSOL(myspace s1, myvars v1, double alfa);
...
int main(void){
  ...
  for(int j = 2;j<=s1.njm;j++){
    sptr->y[j]=s1.y[j-1]+dy;
    dy=dy*exy;
  }
  /*COORDINATES OF CELL CENTERS*/
 
  sptr->xc[1]=sptr->x[1];
  sptr->xc[s1.ni]=sptr->x[s1.nim];
  for(int i = 2;i<=sptr->nim;i++)
    {sptr->xc[i]=0.5*(s1.x[i]+s1.x[i-1]);}
...
void GSS(myspace s1, myvars v1, double alfa){
  ...
  for(int n = 1; n<=v1.maxit;n++){
	...
	myspace * pts1;
	pts1 = &s1;
	res = pts1->q[ij]-s1.ap[ij]*s1.fi[ij]-			\
	  s1.ae[ij]*s1.fi[ij+s1.nj]-s1.aw[ij]*s1.fi[ij-s1.nj]-\
	  s1.as[ij]*s1.fi[ij-1]-s1.an[ij]*s1.fi[ij+1];
...
void SIPSOL(myspace s1, myvars v1, double alfa){
  ...
  for(int i = 2; i<=s1.nim; i++){
    for(int ij = s1.li[i]+2; ij <= s1.li[i]+s1.njm; ij++){
      lw[ij]= s1.aw[ij]/(1.0f+alfa*un[ij-s1.nj]);
      ls[ij] = s1.as[ij]/(1.0f+alfa*ue[ij-1]);
      p1 = alfa*lw[ij]*un[ij-s1.nj];
      p2 = alfa*ls[ij]*ue[ij-1];
      lpr[ij] = (1.0)/(s1.ap[ij]+p1+p2-lw[ij]*ue[ij-s1.nj]-ls[ij]*un[ij-1]);
      un[ij]= (s1.an[ij]-p1)*lpr[ij];
      ue[ij] = (s1.ae[ij]-p2)*lpr[ij];
    }
  }
  ...
  for(int n = 1;n<=v1.maxit; n++){
    ...
    for (int i =2; i<=s1.nim; i++){
      for(int ij = s1.li[i]+2; ij<=s1.li[i]+s1.njm;ij++){
	res[ij]= s1.q[ij]-s1.ap[ij]*s1.fi[ij]-s1.an[ij]*s1.fi[ij+1] - s1.as[ij]*s1.fi[ij-1]-s1.ae[ij]*s1.fi[ij+s1.nj]-s1.aw[ij]*s1.fi[ij-s1.nj];
	resn = resn+fabs(res[ij]);
	res[ij]=(res[ij]-ls[ij]*res[ij-1]-lw[ij]*res[ij-s1.nj])*lpr[ij];
	  }
    }
    if(n == 1){v1.res0=resn;}
  }
